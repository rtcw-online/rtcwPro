	...
	if ( enemyvisible ) {
		...
		if ( aim_skill > 0.6 && wi.proj.damagetype & DAMAGETYPE_RADIAL ) {
			...
			if ( entinfo.origin[2] < bs->origin[2] + 16 ) {
				...
				if ( Q_fabs( trace.endpos[2] - groundtarget[2] ) < 50 ) {
...
void CheckButtons(void)
{
	...
	for (ent = entities; ent; ent = ent->next)
	{
		...
		if (!strcmp(classname, "func_button"))
		{
			...
			dist = Q_fabs(movedir[0]) * size[0] + Q_fabs(movedir[1]) * size[1] + Q_fabs(movedir[2]) * size[2];// - lip;
			dist *= 0.5;
			//
			...
			else
			{
				...
				for (i = 0; i < 3; i++)
				{
					if (movedir[i] < 0) dist += Q_fabs(movedir[i]) * Q_fabs(bboxmaxs[i]);
					else dist += Q_fabs(movedir[i]) * Q_fabs(bboxmins[i]);
				} //end for
				//calculate the goal origin
				VectorMA(origin, -dist, movedir, goalorigin);
				//
...
//		BotAI_Print(PRT_MESSAGE, "%s: I've got no brain cells for activating entities\n", netname);
	   ...
	   if (!strcmp(classname, "func_door"))
	   {
		...
		if ( !strcmp( classname, "func_button" ) ) {
			...
			VectorScale( origin, 0.5, origin );
			//touch distance of the button
			dist = Q_fabs( movedir[0] ) * size[0] + Q_fabs( movedir[1] ) * size[1] + Q_fabs( movedir[2] ) * size[2];
			dist *= 0.5;
			//
			...
			else
			{
				...
				{
					if ( movedir[i] < 0 ) {
						dist += Q_fabs( movedir[i] ) * Q_fabs( bboxmaxs[i] );
					} else { dist += Q_fabs( movedir[i] ) * Q_fabs( bboxmins[i] );}
				} //end for
				  //calculate the goal origin
				VectorMA( origin, -dist, movedir, goalorigin );
